...
export const Doctors: React.FC = () => {
  ...
  const [vets, setVets] = React.useState<Vet[]>([]);
  ...
  React.useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        const mappedVets = res.data.map((doc: any) => ({
          id: doc.id,
          name: doc.name,
        ...
        console.warn('Error fetching doctors, using fallback mock:', err);
        setVets([
          { id: 1, name: 'Dr. John Doe', specialty: 'Khám bệnh đa khoa & Nội khoa', experienceYears: '5 năm', rating: 4.8, initial: 'JD', description: 'Chuyên gia chẩn đoán lâm sàng nội khoa, khám tổng quát và điều trị các bệnh truyền nhiễm thường gặp.', image: '/images/doctor_john.jpg' },
          { id: 2, name: 'Dr. Sarah Conner', specialty: 'Phẫu thuật ngoại khoa & Chấn thương', experienceYears: '8 năm', rating: 4.9, initial: 'SC', description: 'Hơn 8 năm kinh nghiệm thực hiện các ca phẫu thuật phức tạp, chấn thương chỉnh hình và cấp cứu thú cưng.', image: '/images/doctor_sarah.jpg' },
          { id: 3, name: 'Dr. Helen Carter', specialty: 'Da liễu & Điều trị nội trú', experienceYears: '6 năm', rating: 4.7, initial: 'HC', description: 'Chuyên điều trị các bệnh về da liễu, nấm, dị ứng và theo dõi sức khỏe vật nuôi nằm viện dài ngày.', image: '/images/doctor_helen.jpg' },
        ]);
      } finally {
        setLoading(false);
      }
            ...
            {vets.map((vet) => (
              ...
              >
                <div className="flex flex-col items-center space-y-4 w-full">
                  {/* Rounded Avatar Image or Initials */}
                  {vet.image ? (
                    <img 
                      src={vet.image} 
                      alt={vet.name} 
                      className="h-20 w-20 rounded-full object-cover border border-slate-100 shadow-inner group-hover:scale-105 transition duration-300"
                    />
                  ) : (
